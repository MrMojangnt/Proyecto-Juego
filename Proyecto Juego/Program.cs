...
$@"         PRODUCTOS             
 ...
 Productos: {empresa.productos[9]}")
        ...
        btcomprar_acciones.Clicked += () =>
        {
            int cantidty = 0;
            bool IsInt = false;
            List<string> lineas = File.ReadAllLines(ManejoDeArchivos.rutaInventario(InvInt)).ToList();
            ...
            if (pd.balance >= (precioAccional*cantidty) && IsInt == true && cantidty > 0)
            {
                ...
                if (pader == false)
                {
                    using (StreamWriter str = new StreamWriter(ManejoDeArchivos.rutaInventario(InvInt), true, Encoding.UTF8))
                    {
                        str.WriteLine(
                ...
                {
                    File.WriteAllLines(ManejoDeArchivos.rutaInventario(InvInt), lineas);
                }
            ...
            for (int i = 2; i < lineas.Count; i++)
            {
                ...
                if (datos[0] == empresa.id.ToString() && IsInt == true )
                {
                    ...
                    if (cantidadActual > 0 && cantidty > 0 && cantidty <= cantidadActual)
                    {
                        ...
                        pd.balance += precioAccional * cantidty;
                        ModificarPartidas.Guardarelbalance();
                        using (StreamWriter save = new StreamWriter(ManejoDeArchivos.rutaPartidas(InvInt), false, Encoding.UTF8))
